#!/usr/bin/env python3
"""
Test script để kiểm tra cả 3 sàn Binance, OKX, Bitget API thật
"""
import requests
import json

def test_all_three_exchanges():
    base_url = "http://localhost:5000"

    print("🧪 Testing ALL 3 Exchanges Real API Integration...")
    print("=" * 70)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'error' in data:
                ...
                print(f"💳 Cross Margin: ${data.get('cross_margin_loan', 0):,.2f}")
    ...
    print("-" * 30)
    try:
        response = requests.get(f"{base_url}/api/exchanges/bitget/positions", timeout=15)

        if response.status_code == 200:
            data = response.json()
            print("✅ Bitget API: SUCCESS!")

            if 'error' in data:
                print(f"❌ Bitget Error: {data['error']}")
            else:
                print(f"💰 Total Balance: ${data.get('total_balance', 0):,.2f}")
                print(f"📈 Futures PnL: ${data.get('futures_pnl', 0):,.2f}")
                print(f"🎯 Staking: ${data.get('staking_total', 0):,.2f}")
                ...
                if data.get('total_balance', 0) == 2800.0 and len(positions) == 2:
                    print("⚠️  WARNING: This might be mock data (typical test values)")
                else:
                    print("✅ SUCCESS: This appears to be REAL Bitget data!")
        else:
            print(f"❌ Bitget API: HTTP {response.status_code}")

    except Exception as e:
        print(f"❌ Bitget API: Exception - {str(e)}")

    print("\n" + "=" * 70)
    print("🎉 Triple Exchange API Test Complete!")
    print("\n💡 What's implemented:")
    ...
    print("🟢 BITGET:")
    print("   ✅ Real-time swap & spot balance")
    print("   ✅ Active futures positions with PnL")
    print("   ✅ Live funding rates")
    print("   🔄 Staking (placeholder)")
    print("   🔄 Launchpad/Pool (placeholder)")

    print(f"\n🌐 View all 3 exchanges: {base_url}/positions")
    print("🔄 Auto refresh every 30 seconds!")
    print("📊 Compare positions and funding rates across exchanges!")

    return True

if __name__ == "__main__":
    test_all_three_exchanges()
